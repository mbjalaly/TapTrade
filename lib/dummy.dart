...
/// Ensures the provided image file is encoded as JPEG.
/// Returns the original file if already JPEG; otherwise writes and returns a converted JPEG copy.
Future<File> ensureJpegFile(File input, {int quality = 85}) async {
  final String lower = input.path.toLowerCase();
  if (lower.endsWith('.jpg') || lower.endsWith('.jpeg')) {
    return input;
  }
  final img.Image? decoded = img.decodeImage(await input.readAsBytes());
  if (decoded == null) return input;
  final String uniqueName = 'converted_${DateTime.now().millisecondsSinceEpoch}.jpg';
  final File converted = File('${input.parent.path}/$uniqueName')
    ..writeAsBytesSync(img.encodeJpg(decoded, quality: quality));
  return converted;
}

/// Merge multiple images into a single JPEG collage grid.
/// - Images are resized to square tiles to keep layout simple and small.
/// - Returns a newly written JPEG file.
Future<File> mergeImagesToSinglePng(
  List<File> files, {
  int columns = 2,
  int tileSize = 300,
  int padding = 8,
  ...
  if (files.isEmpty) throw ArgumentError('No files to merge');
  // Decode and normalize to square tiles
  final List<img.Image> tiles = [];
  for (final f in files) {
    final bytes = await f.readAsBytes();
    final img.Image? decoded = img.decodeImage(bytes);
    if (decoded != null) {
      final img.Image square = img.copyResize(decoded, width: tileSize, height: tileSize);
      tiles.add(square);
    }
  }
  if (tiles.isEmpty) {
    // Fallback to first file if decoding somehow failed
    return files.first;
  }
  final int count = tiles.length;
  final int cols = columns.clamp(1, 4);
  final int rows = ((count + cols - 1) / cols).ceil();
  final int canvasWidth = cols * tileSize + (cols + 1) * padding;
  final int canvasHeight = rows * tileSize + (rows + 1) * padding;
  final img.Image canvas = img.Image(width: canvasWidth, height: canvasHeight);
  // Fill background white
  img.fill(canvas, color: img.getColor(255, 255, 255));
  for (int i = 0; i < count; i++) {
    final int row = i ~/ cols;
    final int col = i % cols;
    final int dx = padding + col * (tileSize + padding);
