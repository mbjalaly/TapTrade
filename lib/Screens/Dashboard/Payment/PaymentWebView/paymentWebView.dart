      ...
      ..setNavigationDelegate(NavigationDelegate(
        ...
        onNavigationRequest: (NavigationRequest request) async {
          ...
          if (request.url.startsWith(
              'https://mbjalaly.pythonanywhere.com/payment/result/')) {
  ...
  Widget returnButton(Size size){
    if(status.isEmpty){
      ...
      return AppButton(
        onPressed: () async {
          try {
            ...
            if (tradeResponseModel != null && success) {
              ...
              String traderId = ((tradeResponseModel.userProduct?.user ?? "") == userController.userProfile.value.data?.id ? tradeResponseModel.otherProduct?.user : tradeResponseModel.userProduct?.user) ?? "";
        ...
        text: "Reveal Trader",
        isLoading: isLoading,
        margin: EdgeInsets.symmetric(horizontal: size.width / 8,vertical: 20),
      );
      }else{
        return AppButton(
          onPressed: () {
